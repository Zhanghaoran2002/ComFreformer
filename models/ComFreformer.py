...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        if self.individual:
            ...
            for i in range(self.channels):
                self.time_proj.append(nn.Linear(self.seq_len, self.d_deep))
        else:
            self.time_proj = nn.Linear(self.seq_len, self.d_deep)
        # Embedding
        ...
        if self.individual:
            ...
            for i in range(self.channels):
                self.freq_proj.append(nn.Linear(self.d_model, self.out_freq).to(torch.cfloat))
        else:
            self.freq_proj = nn.Linear(self.d_model, self.out_freq).to(torch.cfloat)
    ...
    def forecast(self, x_enc):
        ...
        if self.individual:
            weights = torch.stack([layer.weight for layer in self.freq_proj], dim=0)  # [D, d_out, d_in]
            biases = torch.stack([layer.bias for layer in self.freq_proj], dim=0)    #[D,d_out]
            freq = torch.einsum('coi,bci->bco', weights, enc_out) + biases 
            freq = freq.permute(0,2,1)
        else:
            freq = self.freq_proj(enc_out).permute(0,2,1)
        return freq
    ...
    def forward(self, x, mask=None):
        ...
        if self.individual:
            weights = torch.stack([layer.weight for layer in self.time_proj], dim=0)  # [D, d_out, d_in]
            biases = torch.stack([layer.bias for layer in self.time_proj], dim=0)    #[D,d_out]
            z = torch.einsum('coi,bic->bco', weights, x) + biases 
            z = z.permute(0,2,1)
        else:
            z = self.time_proj(x.permute(0,2,1)).permute(0,2,1)
        ...
        if self.d_deep>0:
            ...
            specx = specxx   
        if self.use_modrelu:
            masks = torch.abs(specx)>1e-2
            specx = specx*masks
            specx = self.activation(specx)
        dec_out = self.forecast(specx)
        low_xy=torch.fft.irfft(dec_out, dim=1)
