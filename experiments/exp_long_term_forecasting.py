...
from models import Informer, Autoformer, Transformer, DLinear, Linear, NLinear, FITS, ComFreformer, iTransformer
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                if self.args.use_amp:
                    with torch.cuda.amp.autocast():
                        ...
                        # outputs = outputs[:, -self.args.pred_len:, f_dim:]
                        batch_y = batch_y[:, -self.args.pred_len:, f_dim:].to(self.device)
                    if self.args.model in ['FITS','ComFreformer']:
                        if self.args.bw==1:
                            loss = criterion(outputs[:, -self.args.pred_len-self.args.seq_len:, f_dim:], batch_xy)
                        else:
                            outputs = outputs[:, -self.args.pred_len:, f_dim:]
                            batch_y = batch_y[:, -self.args.pred_len:, f_dim:].to(self.device)
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                if i % 20 == 0:
                    ...
                    visual(gt, pd, 'ComFreformer', os.path.join(folder_path, str(i) + '.pdf'))
